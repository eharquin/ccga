...
| right complement ✅ | $A^{c}$ s.t. $A\wedge A^{c}=I$ (paper §6.1); $A\wedge A^{c}=(\sum_E A_E^2)I$, $(A^c)^c=-A$ | `right_complement(A)` |
...
**Complement-based PSE↔pencil calculus** ✅ (paper Th 8–11, via $A^{c}$;
`tests/test_pencil_calculus.py`):

| operation | formula | effect |
|---|---|---|
| add a conic (Th 8) | $P\vee C$ | pencil generated by $P$ and $C$ (order $+1$) |
| add a point (Th 9) | $P\wedge p$ | sub-pencil of conics of $P$ through $p$ |
| remove a conic (Th 11) | $P\wedge C^{c}$ | conics of $P$ orthogonal to $C$ |
| remove a point (Th 12) | $P\vee p^{c}$ | drop the $p$ constraint — **open**, see below |
| key identity (Th 10) | $(A\vee B)\wedge B^{c}\equiv A$ | for orthogonal blades |

Verified on the paper's "pencil of three conics" example: $P=p_1\wedge p_2\wedge p_3\wedge I_o^{\triangleright}$
(3-pencil), $C_r=P\wedge p_4\wedge p_5$, $C_g=P\wedge C_r^{c}\wedge p_6$,
$C_b=P\wedge C_r^{c}\wedge C_g^{c}$. $C_r,C_g,C_b$ are pairwise Def-12-orthogonal,
$P\equiv C_r\vee C_g\vee C_b$, and Th 8–11 all check out on this example.

**Th 12 open**: $(A\wedge B)\vee B^{c}\equiv A$ (the identity Th 12's proof needs, with
$A=P'$, $B=p$, $P=A\wedge B$) does **not** reduce to $A$ for real pencils. A
grade-counting argument shows why it *can't*: $p\in\mathbb{NO}(P)$ forces
$gr(P')\ge2$, so $gr(P'\wedge p^{c})=gr(P')+7>8$ is *always* zero for trivial
(dimension) reasons — never because $P'$ and $p$ have disjoint null-basis support
(the genuine Def-12 orthogonality Th 10 actually needs). Concretely,
$(p_1\wedge p_2\wedge p_3\wedge I_o^{\triangleright}\wedge p_4)\vee p_5^{c}\not\equiv
p_1\wedge p_2\wedge p_3\wedge I_o^{\triangleright}\wedge p_4$.

**Norm & orthogonality** ✅ (paper §6.1): $\;\lVert A\rVert=\sqrt{(A\wedge A^{c})^{*}}=\sqrt{\textstyle\sum_E A_E^2}$
(`norm`/`norm2`); $\;A\perp B \iff A\wedge B^{c}=0$ (`orthogonal`). (Fixes the defective
$\sqrt{\tilde A\cdot A}$, which drops the $c$/$e_{o_3}$ term.) This $A\wedge B^c=0$
notion of "orthogonal" does **not** coincide with the classical perpendicular-circles
relation $d^2=r_1^2+r_2^2$ — but it *is* geometrically meaningful: it is exactly the
orthogonality of the $C_r,C_g,C_b$ basis above, extracted from a pencil via $A^c$
rather than from arbitrary circle pairs.
...
**Center & discriminants via meet of three dual lines** ✅ (paper §7):
...
$\Delta_2=ab-\tfrac{c^2}4$, $\Delta_3=abf+\tfrac{cde-c^2f-bd^2-ae^2}4=\det M_3$ — both
recovered as GA scalars: `conic_center_meet`$\to(\Delta_2,x_c,y_c)$,
`conic_discriminant2`$\to\Delta_2$, `conic_discriminant3`$\to\Delta_3$.
`conic_is_degenerate`/`conic_center_point` now use these directly — no `[num]`
linear-algebra fallback remains.
...
|---|---|---|
| ✅ Right complement $A^{c}$ | $A\wedge A^{c}=I$ (paper §6.1) | **done** — `right_complement` (`ccga/operations.py`), `tests/test_complement.py` |
| ✅ Norm / orthogonality | $\lVert A\rVert=\sqrt{(A\wedge A^{c})^{*}}$; $A\wedge B^{c}=0$ | **done** — `norm`/`norm2`/`orthogonal`; norm matches $4A^2{+}4B^2{+}C^2{+}D^2{+}E^2{+}F^2/2$ for canonical conics. $A\wedge B^c=0$ ≠ classical perpendicular-circles ($d^2=r_1^2+r_2^2$), but is geometrically meaningful as the orthogonality of a pencil's $C_r,C_g,C_b$ basis (§4) |
| ✅ $\Delta_2,\Delta_3$, center | meet of three lines (§7) | **done** — `conic_center_meet`/`conic_discriminant2`/`conic_discriminant3` (`ccga/classify.py`); `conic_is_degenerate`/`conic_center_point` no longer use `[num]`; `tests/test_conic_meet_lines.py` |
...
| ✅ Pencil calculus (Th 8–11) | $P\vee C$, $P\wedge p$, $P\wedge C^{c}$ (Th 8,9,11), $(A\vee B)\wedge B^c\equiv A$ (Th 10) | **done** — verified on the "pencil of three conics" example, `tests/test_pencil_calculus.py`. **Th 12** ($P\vee p^c$, remove a point) remains **open** — see §4 for the grade-counting argument why it doesn't reduce to $P'$ |
